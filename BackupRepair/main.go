...
func formatMissing(missing []missingPrimary) string {
	const maxDisplay = 50
	if len(missing) <= maxDisplay {
		items := make([]string, len(missing))
		for i, m := range missing {
			items[i] = fmt.Sprintf("%d", m.bseq)
		}
	...
	for i := 0; i < half; i++ {
		head[i] = fmt.Sprintf("%d", missing[i].bseq)
		tail[i] = fmt.Sprintf("%d", missing[len(missing)-half+i].bseq)
	}
	return fmt.Sprintf("%s, ... (%d more) ..., %s",
		strings.Join(head, ", "), len(missing)-maxDisplay, strings.Join(tail, ", "))
}
...
func main() {
	...
	log.Printf("admin endpoint: %s  sproxyd: %s%s",
		cfg.AdminEndpoint, cfg.SproxydEndpoint, cfg.SproxydPath)

	admin := NewAdminClient(cfg.AdminEndpoint)
