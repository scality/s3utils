...
func NewAdminClient(endpoint, raftSessionID string) *AdminClient {
	return &AdminClient{
		endpoint:      endpoint,
...
// backupsURL returns the bucketd URL prefix for backup operations on this raft session.
func (a *AdminClient) backupsURL() string {
	return fmt.Sprintf("%s/_/raft_sessions/%s/backups", a.endpoint, a.raftSessionID)
}
...
func (a *AdminClient) getBackupIndexPage(copyNumber, minBseq, limit, maxBseq int) ([]BackupIndexEntry, error) {
	url := fmt.Sprintf("%s?copy=%d&limit=%d&minBseq=%d",
		a.backupsURL(), copyNumber, limit, minBseq)
	if maxBseq > 0 {
