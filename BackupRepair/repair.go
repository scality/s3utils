...
func copyKey(sproxyd *SproxydClient, srcKey, dstKey string, bseq, donorCopy int) (bool, error) {
	...
	if err := sproxyd.Put(dstKey, body, info); err != nil {
		return false, err
	}
	return true, nil
}
...
func verifyRepairs(admin *AdminClient, missing []missingPrimary) {
	...
	primarySet := make(map[int]struct{}, len(entries))
	for _, e := range entries {
		primarySet[e.Bseq] = struct{}{}
	}

	var stillMissing []int
	for _, m := range missing {
		if _, ok := primarySet[m.bseq]; !ok {
			stillMissing = append(stillMissing, m.bseq)
		}
	}
	...
	if len(stillMissing) == 0 {
		...
		log.Printf("verification: %d / %d primaries still missing after repair: %s",
			len(stillMissing), len(missing), formatBseqs(stillMissing))
	}
}

func missingBseqs(missing []missingPrimary) []int {
	bseqs := make([]int, len(missing))
