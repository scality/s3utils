 ...
 * @param {string} httpMessage - HTTP status message
 * @param {string} httpDesc - An extra msg to include in the JSON response
...
app.use((req, res, next) => {
    ...
    req.log = log.newRequestLogger(reqUids.map(id => id.slice(0, 128)));
    ...
    req.log.addDefaultFields(defaultInfo);
...
app.use('/api', (req, res, next) => {
    ...
    // timingSafeEquals requires Buffers of equal length for comparison
    // We hash the header value to normalize the length
    // and get a Buffer as a bonus
